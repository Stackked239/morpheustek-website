...
/** false while server-rendering + hydrating, true after — gates the `document.body` portal. */
const noopSubscribe = () => () => {};
...
export function MobileNav({ nav, categories, resources }: { nav: Item[]; categories: Cat[]; resources: Item[] }) {
  ...
  // The overlay must be portalled to <body>: the sticky <header> uses backdrop-filter,
  // which makes it the containing block for `position: fixed` descendants — inside it,
  // `fixed inset-0` sizes and clips to the 72px header band instead of the viewport,
  // so the panel body is invisible on mobile.
  ...
  const overlay = (
    <div
      className={cn(
        ...
        "fixed inset-0 z-[60] overflow-hidden transition-opacity duration-200 lg:hidden",
